...
async def lifespan(app: FastAPI):
    ...
    logger.info("Dataset listo. Zonas: %s | FAIL_RATE=%.0f%%",
                {z: len(DATA[z]) for z in DATA}, FAIL_RATE * 100)
    yield
...
def q1_count(zone_id, confidence_min):
    return sum(1 for b in DATA[zone_id] if b.confidence >= confidence_min)

def q2_area(zone_id, confidence_min):
...
def q3_density(zone_id, confidence_min):
    return q1_count(zone_id, confidence_min) / ZONE_AREAS[zone_id]
...
def q5_confidence_dist(zone_id, bins):
    scores = [b.confidence for b in DATA[zone_id]]
    if not scores:
        return []
    step = 1.0 / bins
    return [
        {"bucket": i, "min": round(i*step, 4), "max": round((i+1)*step, 4),
         "count": sum(1 for s in scores if i*step <= s < (i+1)*step)}
        for i in range(bins)
    ]
...
def handle_query(req: QueryRequest):
    if req.zone_id not in ZONES:
        raise HTTPException(status_code=400, detail=f"Zona invalida: {req.zone_id}")
    if req.query_type not in QUERY_HANDLERS:
        raise HTTPException(status_code=400, detail=f"Tipo de consulta invalido: {req.query_type}")
    if req.query_type == "Q4" and req.zone_id_b not in ZONES:
        raise HTTPException(status_code=400, detail="Q4 requiere zone_id_b valido")

    if FAIL_RATE > 0 and random.random() < FAIL_RATE:
        logger.warning("FALLO SIMULADO para %s:%s", req.query_type, req.zone_id)
